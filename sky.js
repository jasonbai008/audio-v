...
var width,
  ...
  canva = document.querySelector(".vh-bg"),
  stars = [];
...
function draw() {
  ...
  for (var t = stars.length, i = 0; i < t; i++) {
    var e = stars[i];
    e.move(), e.fadeIn(), e.fadeOut(), e.draw();
  }
...
function Star() {
  (this.reset = function () {
    (this.giant = getProbability(3)),
      ...
      (this.do = getRandInterval(5e-4, 0.002) + 0.001 * (this.comet + 1 - 1));
  }),
    ...
    (this.draw = function () {
      ...
      VH_STARRY_SKY.closePath(), VH_STARRY_SKY.fill();
    }),
    (this.move = function () {
      (this.x += this.dx),
        (this.y += this.dy),
